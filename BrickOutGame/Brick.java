...
public class Brick extends Model {
    ...
    public String getDirection(MovingBall ball) {
        ...
        if (brickRect.intersects(ballRect)) {
            int overlapX = Math.min(brickRect.x + brickRect.width, ballRect.x + ballRect.width)
                    - Math.max(brickRect.x, ballRect.x);
            int overlapY = Math.min(brickRect.y + brickRect.height, ballRect.y + ballRect.height)
                    - Math.max(brickRect.y, ballRect.y);
